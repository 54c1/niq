// Compactor: how a reason-family worker compresses its transcript under window
// pressure.
//
// reason only ever requires Compact — the operation invoked when the context
// is over budget or the provider rejects an over-long context. Additional
// operations (e.g. Rotate) are Compactor-specific extensions, reached by type
// assertion from whichever worker declared the corresponding tool; they are
...
// Compactor compresses a transcript. Compact is the one operation reason is
// guaranteed to need on window pressure; it runs under the caller's lock.
...
// The caller holds w.mu; the summarize call runs without it.
