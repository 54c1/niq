...
func TestSoftBudgetInjectsReminder(t *testing.T) {
	...
	if got := len(w.contextBuilder.Render()); got != 1 {
...
func TestCompactToolClosesEpisode(t *testing.T) {
	...
	w.contextBuilder.Apply(builder.AssistantOutput{Message: llm.Message{
	...
	w.contextBuilder.Apply(builder.ToolPlaceholders{Calls: []llm.ContentBlock{
